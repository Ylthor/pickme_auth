...
function TimezoneSelector() {
    const {tz, setTz} = useSiteAttributeStore((store) => store);
    const [selectedKeys, setSelectedKeys] = useState(new Set([DEFAULT_TZ]));
    ...
    return (
        <div className={'flex w-full items-center'}>
            <Dropdown placement="bottom-end">
                <DropdownTrigger>
                    <div className={`${menuItemClassName} grid grid-cols-2`}>
                        <div>Timezone:</div>
                        <div className={'whitespace-nowrap max-w-full text-ellipsis overflow-hidden text-right'}>{TZ_OPTIONS.find((item) => item.value === selectedValue)?.label}</div>
                    </div>
                </DropdownTrigger>
